...
/**
 * PetriNet class stores reference to the root subnet
 * and manages a view of currently opened subnet in form of a stack.
 * Default view is only the root subnet opened.
 * Opening and closing subnets does not influence anything other and serves only for informational purposes.
...
public class PetriNet {
	...
	/**
	 * Returns the root subnet of the Petri net.
	 * It is the only commonly used method.
	...
	/**
	 * Replaces the root subnet with a different one and thus destroys old reference.
	 * Currently useful only for DocumentImporter.
	...
	/**
	 * Replaces the root subnet with empty one and resets view so that opened subnet is the new root subnet.
	 */
	public void clear() {
	...
	/**
	 * Opens a subnet. Changes view, so that specified subnet is currently opened.
	 * The specified subnet must be directly nested in currenly opened subnet.
	...
	/**
	 * Returns a ordered collection of currently opened subnets,
	 * i.e. a path to the currently opened subnet.
